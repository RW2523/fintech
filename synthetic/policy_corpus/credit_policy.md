...
The rules below apply to every financing product the cooperative offers. A product sheet may add to them; none may relax them.

## How a case is decided

A case passes through the same sequence every time, and the order is not a preference. Hard gates are checked first, because a case that cannot proceed should not be scored. Evidence validity is checked next, because a score built on a document nobody could read is not a score. Only then are the Decision Factors weighted, and only then is the case routed.

A model never decides. It produces a number, that number is an input to a factor, and the factors are combined by a formula that is written down and versioned. Where a decision was close, the record says which factor decided it and what would have changed the outcome.
...
Conditions the Shariah board sets for this product. A breach stops the case; it is not traded off against a strong score.

### SHR-01

*Shariah compliance: purpose not permitted.*

**Condition.** The case satisfies this clause when `in(requested.purpose, product.purposes_allowed)`.

**If it does not.** The application cannot proceed on these terms, under reason code `SHR-02`.

**Inputs.** The clause is evaluated from `requested.purpose`. Every one of them carries evidence back to the record it came from.

### SHR-03

*Shariah compliance: structure compliant.*

**Condition.** The case satisfies this clause when `product.structure == 'MURABAHAH'`.

**If it does not.** The case leaves the normal path and goes to a person, under reason code `SHR-03`.

**Inputs.** The clause is evaluated from `product.structure`. Every one of them carries evidence back to the record it came from.

### SHR-04

*Shariah compliance: structure compliant.*

**Condition.** The case satisfies this clause when `restructure.type != 'RATE_INCREASE'`.

**If it does not.** The application cannot proceed on these terms, under reason code `SHR-03`.

**Inputs.** The clause is evaluated from `restructure.type`. Every one of them carries evidence back to the record it came from.
