"""T-008 — member_intelligence answers the platform's meta endpoints."""
...
async def _client() -> AsyncClient:
    return AsyncClient(transport=ASGITransport(app=app), base_url="http://member_intelligence")
...
async def test_health_reports_ok_with_the_service_name() -> None:
    ...
    assert body == {"status": "ok", "service": "member_intelligence", "version": "0.1.0"}


async def test_version_reports_the_environment() -> None:
    async with await _client() as http:
        body = (await http.get("/version")).json()
    assert body["service"] == "member_intelligence"
