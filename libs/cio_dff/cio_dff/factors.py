...
def _finish(
    ...
    *,
    ...
    """Package a score with the identifier and digest that make it citable.

    The calculation id is normally minted fresh, because computing a score is
    an event and two identical computations are still two events. A caller
    that must be able to re-derive the whole record from its inputs -- the
    risk service re-scoring a frozen snapshot, say -- asks for a derived id
    instead, so replaying the snapshot reproduces the reference as well as the
    number.
    """
    digest = sha256(canonical_json(inputs))
    calc_id = derived_id("calc", family, digest) if derived else new_id("calc")
    return FactorScore(
        family=family,
        score=_clamp(score),
        calc_id=calc_id,
...
def conduct_score(
    *,
    ...
    if history_months < 6:
        ...
        return _finish("CONDUCT", 55, inputs, evidence_refs, derived=derived)
    ...
    return _finish("CONDUCT", value, inputs, evidence_refs, derived=derived)
